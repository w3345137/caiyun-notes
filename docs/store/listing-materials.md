...
- 审核测试账号尚未验证，不得把示例凭据提交给 Microsoft；如审核需要，应创建独立的低权限测试账号，验证登录后仅在 Partner Center 的非公开测试信息中填写。
- 注明：笔记数据云端同步，需要联网；应用内部分功能（邮箱、网盘、大模型、地图）需要用户自行配置第三方服务凭据，不配置不影响核心笔记功能。
- 应用为桌面桥全信任进程（Tauri/WebView2）；原生壳不自更新，壳更新由 Microsoft Store 分发；已审核功能范围内的签名前端资源仍可热更新。
