...
import { AlertCircle, Plus, Minus, Table as TableIcon, ChevronDown, PaintBucket, Lock, Unlock } from 'lucide-react';
    ...
    <div ref={toolbarRef} className={`relative z-10 flex items-center gap-[2px] px-1 py-0.5 ${themeClass} rounded-lg shadow-md border border-gray-200 ${disabled ? 'opacity-70 pointer-events-none select-none' : ''}`}>
      ...
      <div className="flex items-center gap-0.5 px-2 border-r border-gray-200">
        ...
        <ToolbarButton onClick={() => editor.chain().focus().toggleItalic().run()} isActive={editor.isActive('italic')} disabled={disabled} title="斜体 (Ctrl+I)">
          <span className="font-bold italic text-[14px] leading-none">I</span>
        </ToolbarButton>
      ...
      <div className="flex items-center gap-0.5 px-2 border-r border-gray-200">
        ...
        <ToolbarButton onClick={() => editor.chain().focus().toggleOrderedList().run()} isActive={editor.isActive('orderedList')} disabled={disabled} title="有序列表">
          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <text x="2" y="9.5" fontSize="10" fontWeight="bold" fill="currentColor" stroke="none" fontFamily="sans-serif">1-</text>
            <text x="2" y="18.5" fontSize="10" fontWeight="bold" fill="currentColor" stroke="none" fontFamily="sans-serif">2-</text>
          </svg>
        </ToolbarButton>
        <ToolbarButton onClick={() => editor.chain().focus().toggleTaskList().run()} isActive={editor.isActive('taskList')} disabled={disabled} title="待办列表 (Ctrl+1) | Tab缩进">
          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <polyline points="4 16 11 21 22 1"/>
        ...
        >
          <svg className="w-5 h-5" viewBox="0 0 24 24">
            ...
            <g transform="translate(12,12) scale(0.85) translate(-12,-12)">
              <line x1="12" y1="5" x2="12" y2="19" stroke="white" strokeWidth="2.3" strokeLinecap="round"/>
              <line x1="5" y1="12" x2="19" y2="12" stroke="white" strokeWidth="2.3" strokeLinecap="round"/>
            </g>
          </svg>
        </ToolbarButton>
        {showInsertDropdown && (
...
export const NoteEditor: React.FC = () => {
  ...
  useEffect(() => {
    const checkSharedStatus = async () => {
      ...
      setIsSharedNotebook(!error && data && data.length > 0);
    };
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: `outline-none ${!canEdit ? 'opacity-60 pointer-events-none select-none' : ''}`,
      },
    },
  ...
  const handleMindmapClick = () => {
    if (editor) {
      ...
      toast.success('思维导图已添加，点击编辑');
    }
    setShowMindmapEditor(false);
  };

  const handleMindmapSave = async (content: string) => {
    setShowMindmapEditor(false);
  };
  ...
  // 渲染思维导图 SVG
  const renderMindmapSVG = (nodes: any[]): string => {
    if (!nodes || nodes.length === 0) return '';
    const node = nodes[0];
    const nodeColor = node.color || '#4a90e2';
    
    const renderNode = (n: any, isRoot: boolean): string => {
      const rx = isRoot ? 90 : 70;
      const ry = isRoot ? 35 : 28;
      const fontSize = isRoot ? 15 : 13;
      const text = (n.text || '').slice(0, isRoot ? 12 : 10) + ((n.text || '').length > (isRoot ? 12 : 10) ? '...' : '');
      let html = `<g><ellipse cx="${n.x}" cy="${n.y}" rx="${rx}" ry="${ry}" fill="${n.color || nodeColor}" filter="url(#shadow)"/><text x="${n.x}" y="${n.y + 5}" text-anchor="middle" fill="white" font-size="${fontSize}" font-weight="${isRoot ? 600 : 500}">${text}</text>`;
      ...
      if (n.children) {
        n.children.forEach((child: any) => {
          ...
          const startX = n.x + Math.cos(angle) * rx;
          const startY = n.y + Math.sin(angle) * ry;
          const endX = child.x - Math.cos(angle) * 70;
          const endY = child.y - Math.sin(angle) * 28;
          html += `<path d="M ${startX} ${startY} Q ${(startX + endX) / 2} ${(startY + endY) / 2 + (dy > 0 ? 25 : -25)} ${endX} ${endY}" stroke="${child.color || nodeColor}" stroke-width="2" fill="none" stroke-linecap="round"/>`;
    ...
    return `<svg width="100%" height="400" viewBox="0 0 800 400"><defs><filter id="shadow" x="-50%" y="-50%" width="200%" height="200%"><feDropShadow dx="0" dy="2" stdDeviation="2" flood-opacity="0.15"/></filter></defs>${renderNode(node, true)}</svg>`;
  };
  ...
  useEffect(() => {
    ...
    const processMindmaps = () => {
      const containers = editor.view.dom.querySelectorAll('.mindmap-node:not([data-processed])');
      containers.forEach((container) => {
        const mindmapData = container.getAttribute('data-mindmap');
        if (!mindmapData) { container.setAttribute('data-processed', 'true'); return; }
        if (container.querySelector('.mindmap-rendered')) { container.setAttribute('data-processed', 'true'); return; }

        try {
          const nodes = JSON.parse(mindmapData);
          if (nodes && nodes.length > 0) {
            const div = document.createElement('div');
            div.className = 'mindmap-rendered';
            div.style.cssText = 'width:100%;min-height:200px;background:linear-gradient(135deg,#f8fafc 0%,#e2e8f0 100%);border-radius:12px;overflow:hidden;position:relative;margin:12px 0;';
            div.innerHTML = renderMindmapSVG(nodes);
            const title = document.createElement('div');
            title.style.cssText = 'position:absolute;top:8px;left:12px;font-size:12px;color:#64748b;';
          ...
          container.innerHTML = '<p style="color:red;padding:16px;text-align:center;">思维导图渲染失败</p>';
        }
        container.setAttribute('data-processed', 'true');
      });
    };

    const timeoutId = setTimeout(processMindmaps, 500);
    editor.on('transaction', processMindmaps);
    return () => { clearTimeout(timeoutId); editor.off('transaction', processMindmaps); };
  ...
  return (
    <div className="flex-1 flex flex-col bg-white relative max-w-full overflow-hidden h-full">
      ...
      {/* 思维导图编辑器 - 使用 Mind Elixir */}
      {showMindmapEditor && (
        <MindElixirEditor
          content=""
          onSave={handleMindmapSave}
          onCancel={() => setShowMindmapEditor(false)}
        />
      )}
    </div>
  );
