...
这个公开仓库只包含彩云笔记 p20 桌面 App 的 Tauri 壳、经过生产构建的内置前端资源和发布工作流，不包含 TypeScript 源码、后端、数据库脚本、江苏公司专属逻辑、HCM 集成或任何生产凭据。
...
p20 使用内置前端入口，并为行程规划提供受限的内嵌地图子 WebView。App 启动时先从本机加载 UI 和缓存，再通过 HTTPS / WSS 连接 `https://notes.binapp.top`。壳更新从国内 OSS 分发，支持持久化断点续传、整包签名校验和稳定设备分批发布；前端热更旧链路仍保留。macOS 红色关闭按钮与 Windows ×/Alt+F4 会隐藏主窗口；macOS 可从 Dock 恢复并通过 Cmd+Q 退出，Windows 可从系统托盘恢复或退出。真正退出前，桌面壳会等待页面、CRDT 日志和本地队列刷入本地库；Windows 再次启动时会恢复已有窗口而不是创建第二个进程。远程网页不获得通用 Tauri 文件、Store、剪贴板、窗口关闭或进程退出权限。
