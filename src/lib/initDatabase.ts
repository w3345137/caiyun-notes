...
export async function initDatabase(): Promise<boolean> {
  try {
    // 本地认证检测：检查是否有有效 token
    const userId = getCurrentUserId();
    if (!userId) {
      console.error('数据库连接失败: 未登录');
      return false;
    }
    return true;
  } catch (e) {
...
export async function saveNoteToCloud(note: Note, expectedVersion?: number): Promise<{ success: boolean; error?: string; conflictInfo?: { serverVersion: number; clientVersion: number } }> {
  ...
  // 锁检查已在 Edge Function 服务端执行，前端不再重复检查
...
export async function getUpdateLogs(): Promise<UpdateLog[]> {
  try {
    ...
    const res = await fetch(`${API_BASE}/update-logs`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({ action: 'getUpdateLogs' })
    });
    const result = await res.json();
    if (!result.success) {
      console.error('[DB] 获取更新日志失败:', result.error);
      return [];
    }
    // items 可能是 JSON 字符串，需要解析
...
export async function updateUpdateLog(
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/update-logs`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({ action: 'updateUpdateLog', id, ...updateData, items: JSON.stringify(updateData.items) })
    });
    const result = await res.json();
    if (!result.success) {
      return { success: false, error: result.error };
...
export async function addUpdateLog(
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/update-logs`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({ action: 'addUpdateLog', ...logData, items: JSON.stringify(logData.items) })
    });
    const result = await res.json();
    if (!result.success) {
      return { success: false, error: result.error };
...
export async function deleteUpdateLog(
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/update-logs`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({ action: 'deleteUpdateLog', id })
    });
    const result = await res.json();
    if (!result.success) {
      return { success: false, error: result.error };
